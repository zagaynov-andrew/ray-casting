...
/*   Updated: 2021/03/18 10:36:16 by ngamora          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float			get_side_dist_x(const t_player *player, const t_vec2 *ray_dir,
															float delta_dist_x)
{
	int		num_line;
	int		line_x;
...
float			get_side_dist_y(const t_player *player, const t_vec2 *ray_dir,
															float delta_dist_y)
{
	int		num_line;
	int		line_y;
...
{
	return (CUB_SIZE * sqrtf(1 + ((float)(ray_dir->y * ray_dir->y))
												/ (ray_dir->x * ray_dir->x)));
}

float			get_delta_dist_y(const t_vec2 *ray_dir)
{
	return (CUB_SIZE * sqrtf(1 + ((float)(ray_dir->x * ray_dir->x))
												/ (ray_dir->y * ray_dir->y)));
}
