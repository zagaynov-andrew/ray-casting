...
/*   Updated: 2021/03/27 14:47:59 by ngamora          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_error_utils(void)
{
	...
	errno == ERR_TEXTURE_NOT_EXIST ? ft_putstr_fd("Texture file \
													not exist\n", 2) : 0;
	errno == ERR_EXTRA_CHAR ? ft_putstr_fd("Extra character in the \
													configuration\n", 2) : 0;
...
int			print_error(void)
{
	...
	errno == ERR_EXTRA_CHAR_AFTER ? ft_putstr_fd("Extra characters \
												after configuration\n", 2) : 0;
	errno == ERR_RSOLUTION ? ft_putstr_fd("The screen resolution is not \
							greater than zero in the config \"R\"\n", 2) : 0;
	print_error_utils();
	return (EXIT_FAILURE);
}
